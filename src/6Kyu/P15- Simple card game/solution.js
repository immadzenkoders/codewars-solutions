...
function winner(deckSteve, deckJosh) {
    ...
    for (let i = 0; i < deckSteve.length; i++) {
        const cardSteve = deckSteve[i];
        const cardJosh = deckJosh[i];
        if (cardValue(cardSteve) > cardValue(cardJosh)) {
            scoreSteve++;
        } else if (cardValue(cardSteve) < cardValue(cardJosh)) {
            scoreJosh++;
        }
    }
...
console.log(winner(['A','7','8'], ['K','5','9'])); // Output: "Steve wins 2 to 1"
